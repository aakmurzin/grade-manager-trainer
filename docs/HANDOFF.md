...
| Слой | Выбор | Почему |
|---|---|---|
| Frontend | **Next.js** (App Router) + TypeScript на **Vercel** | Несколько стейтфул-экранов (логин, история, radar-chart отчёта) — DOM-манипуляции как в прототипе здесь не масштабируются. Next на Vercel — один деплой с API routes |
| Backend | **Свой, отдельный от Grade-монолита** (Laravel Grade не трогаем вообще на этом этапе): **Neon Postgres** + **Drizzle** | Явное решение — не грузить Grade-девелопера. Изначально в спеке фигурировал Supabase как разумный дефолт («минимум инфраструктурной работы»); фактически подняты Neon + Auth.js — переписывать под старый текст не нужно (addendum-66/67) |
| Auth (Phase 1) | **Auth.js** Credentials, email/password (без magic link) | Свой независимый логин, ничего не ждём от Grade |
...
| Персонажи/объекты | Спрайт-листы (см. `asset-prompts.md`), рендер через PixiJS / canvas | Уход от CSS-фигур прототипа — там это было приемлемо для однофайлового HTML, здесь не обязательно тащить то же ограничение |

---

## 4. Схема БД (Neon / Postgres)
...
**Phase 1 (сейчас):**
```
Signup/Login (email+password, Auth.js Credentials → Neon)
