/**
 * Compare Addendum 19 Design Classical vs Addendum 17 baseline.
 */
...
function analyze(sessions: Session[]) {
  ...
  return {
    ...
    bestCum: Math.max(
      ...sessions.map((s) => {
        const qs = s.quarterNetProfit ?? [];
        return qs.length >= 4 ? qs.reduce((a, b) => a + b, 0) : -Infinity;
      }),
    ),
    rows: sessions.map((s) => {
      const qs = s.quarterNetProfit ?? [];
...
function main() {
  const a19Name = process.argv[2];
  if (!a19Name) {
    console.error('Usage: tsx analyze-addendum-19.ts <batch-a19.json>');
    process.exit(1);
  }
  const a17 = load('batch-addendum-17-classical-design.json');
  const a19 = load(a19Name);
  const summary = {
    a17_design: analyze(a17.sessions),
    a19_design: analyze(a19.sessions),
  };
  const out = join(RESULTS, 'addendum-19-classical-summary.json');
